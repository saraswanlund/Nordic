...
static int epson_s1d13541_i2c_read(struct pl_i2c *i2c, uint8_t i2c_addr,
				   uint8_t *data, uint8_t count,
				   uint8_t flags);
static int epson_s1d13541_i2c_write(struct pl_i2c *i2c, uint8_t i2c_addr,
				    const uint8_t *data, uint8_t count,
				    uint8_t flags);
static int s1d13541_i2c_send_addr(struct s1d13541 *p, uint8_t i2c_addr,
				  uint8_t read);
static int s1d13541_i2c_poll(struct s1d13541 *p, int check_nak);

/*
 *   Initialization of the I2C Module
 */
//int epson_i2c_init(struct s1d13541 *p, struct pl_i2c *i2c,
//		   enum epson_epdc_ref ref)
//{
//	if (epson_epdc_early_init(p, ref))
//		return -1;
//
//	i2c->read = epson_s1d13541_i2c_read;
//	i2c->write = epson_s1d13541_i2c_write;
//	i2c->priv = p;
//
//	return 0;
//}
...
//{
//	struct s1d13541 *p = i2c->priv;
//
//	if (!(flags & PL_I2C_NO_START))
//		if (s1d13541_i2c_send_addr(p, i2c_addr, 1))
//			return -1;
//
//	while (count--) {
//		uint8_t cmd;
...
//			cmd = S1D135XX_I2C_CMD_GO | S1D135XX_I2C_CMD_READ;
//
//		s1d13541_write_reg(p, S1D135XX_I2C_REG_CMD, cmd);
//
//		if (s1d13541_i2c_poll(p, 0))
//			return -1;
//
//		*data++ = s1d13541_read_reg(p, S1D135XX_I2C_REG_RD);
//	}
//
//	return 0;
//}
...
//
//	if (!(flags & PL_I2C_NO_START))
//		if (s1d13541_i2c_send_addr(p, i2c_addr, 0))
//			return -1;
//
//	while (count--) {
//		uint8_t cmd;
...
//
//		s1d13541_write_reg(p, S1D135XX_I2C_REG_WD, *data++);
//		s1d13541_write_reg(p, S1D135XX_I2C_REG_CMD, cmd);
//
//		if (s1d13541_i2c_poll(p, 1))
//			return -1;
//	}
//
//	return 0;
...
//				  uint8_t read)
//{
//	s1d13541_write_reg(p, S1D135XX_I2C_REG_WD, ((i2c_addr) << 1) | read);
//	s1d13541_write_reg(p, S1D135XX_I2C_REG_CMD, (S1D135XX_I2C_CMD_START |
//						     S1D135XX_I2C_CMD_GEN |
//						     S1D135XX_I2C_CMD_GO));
//
//	return s1d13541_i2c_poll(p, 1);
//}
//
//static int s1d13541_i2c_poll(struct s1d13541 *p, int check_nak)
//{
...
//
//	while (--i) {
//		status = s1d13541_read_reg(p, S1D135XX_I2C_REG_STAT);
//
